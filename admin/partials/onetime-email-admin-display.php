...
global $wp_roles;
$option_name = "couponemails_onetimeemail";

if ( isset( $_GET['runtest'] ) ) {
	$onetime = new Coupon_Emails_Onetimes();
	$result = $onetime->send_to_users_filtered($option_name);
	header("location:admin.php?page=couponemails&tab=one-time&section=no-coupon");
}
...
if ( isset( $_GET['onetimeexport'] ) ) {
	...
	$onetime = new Coupon_Emails_PrepareSQL($option_name, '<=');
	$result = $onetime->get_users_filtered();
...
<div class="couponemails_loader_cover">
	...
	<ul class="subsubsub">
		<li>
			<a href="?page=couponemails&tab=one-time" class="">
			<?php echo  esc_html__( 'Email with coupon', 'coupon-emails' ); ?></a>
			|</li>
		<li>
			<a href="?page=couponemails&tab=one-time&amp;section=no-coupon" class="current">
...
<br>
<h2><?php echo esc_html_x('One Time Emails Settings','Setting', 'coupon-emails'); ?> </h2>
<h4><?php echo esc_html_x('Use the settings in this section to filter out users who will be sent a one-time email without a coupon.','Setting', 'coupon-emails'); ?> </h4>
<form method="post" id="form3" name="form3" action="options.php">
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				<a class="button button-primary" href="admin.php?page=couponemails&tab=one-time&section=no-coupon&onetimeexport=table&noheader=1"><?php echo esc_html__( 'Download csv', 'coupon-emails' ); ?></a>
